...
public class ItemTouchHelperRecyclerViewProjects extends ItemTouchHelper.SimpleCallback {
    ...
    public void onSwiped(RecyclerView.ViewHolder viewHolder, int direction) {
        ...
        if (direction == ItemTouchHelper.LEFT) {
            // delete
            deletedProject = ProjectManager.projects.get(INDEX);
            ProjectManager.projects.remove(INDEX);
            ...
            Snackbar.make(projectsFragment.getRecyclerView(), "Kunde \"" + deletedProject.getName() + "\" gelöscht", Snackbar.LENGTH_LONG)
                    .setAction("Rückgängig machen", new OnClickListenerUndoDeleteCustomer(recyclerAdapterProjects, INDEX, deletedProject))
                    .show();
        }
        else if (direction == ItemTouchHelper.RIGHT) {
            // archive
            archivedProject = ProjectManager.projects.get(INDEX);
        ...
        recyclerAdapterProjects.setProjects(ProjectManager.projects);
        system.saveData();
    }
